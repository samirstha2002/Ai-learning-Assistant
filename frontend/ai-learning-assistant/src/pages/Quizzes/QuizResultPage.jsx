  ...
  return (
    <div className="max-w-5xl mx-auto ">
      ...
      <div className="mb-6">
        ...
        >
          <ArrowLeft
            className="w-4 h-4 group-hover:-translate-x-1 transition-transfprm duration-200"
            strokeWidth={2}
          />
          Back to document
        </Link>
      ...
      <div className="space-y-6">
        ...
        {detailedResults.map((result, index) => {
          ...
          return (
            <div
              className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl p-6 shadow-lg shadow-sky-200/50"
              key={index}
            >
              <div className="flex itens-start justify-between gap-4 mb-3">
              ...
              <div className="space-y-3 mb-4">
                {result.options.map((option, optIndex) => {
                  ...
                  return (
                    <div
                      key={optIndex}
                      className={`relative px-4 py-3 rounded-lg border-2 transition-all duration-200 ${isCorrectOption ? "bg-emerald-50 border-emerald-300 shasow-lg shadow-emerald-500/10" : isWrongAnswer ? "bg-rose-50 border-rose-300" : "bg-slate-50 border-slate-200"}`}
                    >
      ...
      <div className="mt-8 flex justify-center">
        <Link to={`/documents/${quiz.document._id}`}>
          <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shaow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden ">
